...
void IRCCmd::Handle_Login(_CDATA *CD)
{
    ...
    if(isbanned == "NOTBANNED")
    {
        if(!IsLoggedIn(CD->USER))
        {
            if(!AcctIsLoggedIn(_PARAMS[0].c_str()))
            {
                ...
                if (result)
                {
                    ...
                    if(GMLevel >= 0)
                    {
                        ...
                        Send_IRCA(CD->USER, MakeMsg("You Are Now Logged In As %s, Welcome To TriniChat Admin Mode.", _PARAMS[0].c_str()), true, CD->TYPE);
         ...
         Send_IRCA(CD->USER, "\0034[ERROR] : Sorry You Are "+isbanned+". You Cannot Log In To TriniChat "+CD->USER.c_str()+"!", true, "ERROR");
}
